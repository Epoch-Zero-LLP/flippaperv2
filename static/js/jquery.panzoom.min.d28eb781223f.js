...
$("input[name=map]").change(function() {
    var test = $(this).val();
    $(".maps-item").removeClass('maps-item--show');
    $("#"+test).addClass('maps-item--show');
});

function maps() {
    var bwidth = $(window).width();
    if (bwidth > 1501) {
            var $section = $('.maps-item');
            var $panzoom = $section.find('.panzoom').panzoom({
                minScale: 0.9,
                maxScale: 4.5,
                increment: 0.2,
                startTransform  : 'translate(-400px, -300px) scale(3.2)',
                contain: 'automatic',
                $zoomIn: $('.maps-controls').find(".maps-controls__zoom-in"),
                $zoomOut: $('.maps-controls').find(".maps-controls__zoom-out"),
                $reset: $('.maps-controls').find(".maps-controls__zoom-reset"),
            });
    ...
    } else if (bwidth < 1500 && bwidth > 1201) {
            var $section = $('.maps-item');
            var $panzoom = $section.find('.panzoom').panzoom({
                minScale: 0.9,
                maxScale: 4.5,
                increment: 0.2,
                startTransform  : 'translate(-400px, -550px) scale(3.5)',
                $zoomIn: $('.maps-controls').find(".maps-controls__zoom-in"),
                $zoomOut: $('.maps-controls').find(".maps-controls__zoom-out"),
                $reset: $('.maps-controls').find(".maps-controls__zoom-reset"),
            });
    ...
    } else if (bwidth < 1200 && bwidth > 901) {
            var $section = $('.maps-item');
            var $panzoom = $section.find('.panzoom').panzoom({
                minScale: 0.9,
                maxScale: 4.5,
                increment: 0.2,
                startTransform  : 'translate(-350px, -40px) scale(4.5)',
                contain: 'automatic',
                $zoomIn: $('.maps-controls').find(".maps-controls__zoom-in"),
                $zoomOut: $('.maps-controls').find(".maps-controls__zoom-out"),
                $reset: $('.maps-controls').find(".maps-controls__zoom-reset"),
    ...
    } else if (bwidth < 900 && bwidth > 766) {
            var $section = $('.maps-item');
            var $panzoom = $section.find('.panzoom').panzoom({
                minScale: 0.9,
                maxScale: 4.5,
                increment: 0.2,
                startTransform  : 'translate(-350px, 1030px) scale(5.5)',
                contain: 'automatic',
                $zoomIn: $('.maps-controls').find(".maps-controls__zoom-in"),
                $zoomOut: $('.maps-controls').find(".maps-controls__zoom-out"),
                $reset: $('.maps-controls').find(".maps-controls__zoom-reset"),
...
$(document).ready(function () {
    ...
    var $section = $('.plan-item');
    var $panzoom = $section.find('.panzoom').panzoom({
        minScale: 1.6,
        maxScale: 5.5,
        increment: 0.2,
        startTransform  : 'translate(10%, -30%) scale(1.6)',
        contain: 'invert',
        $zoomIn: $('.maps-controls').find(".maps-controls__zoom-in"),
        $zoomOut: $('.maps-controls').find(".maps-controls__zoom-out"),
        $reset: $('.maps-controls').find(".maps-controls__zoom-reset"),
